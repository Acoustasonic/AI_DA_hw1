...
def mcts_agent(game_state):
    ...
    try:
        ...
        agent = MCTSAgent(num_rounds=60, rollout_depth=20)
        return agent.select_move(game_state)
...
def play_game(agent1_fn, agent2_fn, board_size=9, verbose=True):
    ...
    while not game.is_over():
        ...
        if move_count > board_size * board_size * 2:
            print("[WARN] 步数过多，按当前盘面强制结算")
            forced_winner = compute_game_result(game).winner
            break

    duration = time.time() - start_time
    winner = forced_winner if forced_winner is not None else game.winner()
